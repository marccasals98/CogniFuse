...
def generate_model_name(params, start_datetime, wandb_run_id = None, wandb_run_name = None):
        ...
        name_components.append(params.speech_feature_extractor)
        name_components.append(params.text_feature_extractor)
        name_components.append(params.speech_adapter)
        name_components.append(params.text_adapter)
...
def get_waveforms_stats(labels_lines, sample_rate):
    ...
    for audio_num, audio_path in enumerate(audio_paths):
        ...
        if original_sample_rate != sample_rate:
            logger.debug(f"resampling from {original_sample_rate} to {sample_rate}")
            waveform, original_sample_rate = librosa.load(audio_path, mono = True, sr = sample_rate)
